...
int main(void) {
    ...
    char name[MAX_WORD_SIZE];
    int age;
    // temporary storage for birthday
    int y, m, d;
    ...
    for(int i = 0; i < N; i++) {
        ...
        // of this person; this is just
        // one way of doing it
        // ideally, you want to create a create_date()
        // function and update the create_person()
        // function so that it accepts the birthday
        // information of the person
        persons[i].birthday.year = y;
        persons[i].birthday.month = m;
        persons[i].birthday.day = d;
    }
...
void introduce(Person p) {
    ...
    // access a field of a structure (Date_t) that is also
    // a field of the current structure (Student)
